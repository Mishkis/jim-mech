...
public class MechGeoModel<T extends GeoEntity> extends DefaultedEntityGeoModel<T> {
    ...
    public void setCustomAnimations(T anim, long instanceId, AnimationState<T> animationState) {
        ...
        if (current_animation == DefaultAnimations.WALK || current_animation == Mech.STAND_ANIMATION || current_animation == Mech.LAND_ANIMATION) {
        ...
        if (animatable.getControllingPassenger() instanceof PlayerEntity player) {
            if (animationState.isCurrentAnimation(Mech.MACHINE_GUN_SHOOT_ANIMATION)) {
            ...
            } else if (animationState.isCurrentAnimationStage("block.grab")) {
                CoreGeoBone left_arm = getAnimationProcessor().getBone("LeftArm");
                float left_arm_rotation = -(player.getPitch() - 90) * MathHelper.RADIANS_PER_DEGREE;
                left_arm.setRotX(left_arm_rotation);
            }
        }
    }
}
